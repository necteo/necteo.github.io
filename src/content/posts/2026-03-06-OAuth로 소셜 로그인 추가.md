...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        RefreshToken saved = refreshTokenRepository.findByMember(member)
                ...
                .orElseGet(() -> RefreshToken.builder().member(member).token(refreshToken).build());
        refreshTokenRepository.save(saved);

        // JWT를 URL 파라미터 대신 HttpOnly 쿠키에 직접 설정
...
여기서 JWT 토큰을 만들어서 **HttpOnly 쿠키**에 직접 설정한다

URL 파라미터로 토큰을 전달하면 브라우저 히스토리, 서버 로그에 토큰이 노출될 수 있어서
HttpOnly 쿠키 방식이 더 안전하다
...
public class JwtTokenProvider {
    ...
    public Jwt validateAndParse(String token) {
        try {
            ...
            if (isExpired) throw new CustomException(ErrorCode.TOKEN_EXPIRED);
            throw new CustomException(ErrorCode.TOKEN_INVALID);
        } catch (BadJwtException | JwtException e) {
            throw new CustomException(ErrorCode.TOKEN_INVALID);
        }
    }
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    // Authorization 헤더 대신 HttpOnly 쿠키에서 토큰 추출
